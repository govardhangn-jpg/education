...
import { SUBJECT_META, SUBJECTS_BY_GRADE, DIFFICULTY_META, LANGUAGES, GRADES, SYLLABI } from '../utils/constants';

export default function QuizPage() {
  const { user } = useAuth();
  const [tab, setTab] = useState('setup');
  const [config, setConfig] = useState({
    grade: user?.grade||'Class 7', syllabus: user?.syllabus||'CBSE',
    subject:'Mathematics', chapter:'', difficulty:'medium', count:5,
  ...
  useEffect(() => {
    if (config.subject && config.grade && config.syllabus)
      getCurriculum({ grade:config.grade, syllabus:config.syllabus, subject:config.subject })
  ...
  return (
    <div style={{ padding:'14px 14px', maxWidth:900, margin:'0 auto', fontFamily:"'Nunito',sans-serif" }}>
      <style>{`
      ...
      {tab==='setup' && (
        <div className="quiz-setup-grid">
          ...
          <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:18, padding:18 }}>
            ...
            {[['Grade','grade',GRADES],['Syllabus','syllabus',SYLLABI],['Language','language',LANGUAGES]].map(([label,key,opts])=>(
              <div key={key} style={{ marginBottom:12 }}>
                <label style={{ color:'rgba(255,255,255,0.6)', fontSize:11, fontWeight:700, display:'block', marginBottom:5 }}>{label.toUpperCase()}</label>
                <select value={config[key]} onChange={e=>set(key,e.target.value)}>{opts.map(o=><option key={o} value={o}>{o}</option>)}</select>
              </div>
            ))}
            ...
            <div style={{ marginBottom:12 }}>
              ...
              <div style={{ display:'flex', gap:8 }}>
                {Object.entries(DIFFICULTY_META).map(([k,v])=><button key={k} onClick={()=>set('difficulty',k)} style={{ flex:1, padding:'9px', background:config.difficulty===k?`${v.bg}`:'rgba(255,255,255,0.05)', border:`1.5px solid ${config.difficulty===k?v.color:'rgba(255,255,255,0.1)'}`, borderRadius:10, color:config.difficulty===k?v.color:'rgba(255,255,255,0.6)', fontSize:12, fontWeight:700, cursor:'pointer' }}>{v.label}</button>)}
              </div>
            </div>
          </div>
          ...
          <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:18, padding:18, display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(90px,1fr))', gap:8, marginBottom:16 }}>
              {subjects.map(sub=>{
                const m=SUBJECT_META[sub]; const active=config.subject===sub;
                return <button key={sub} onClick={()=>set('subject',sub)} style={{ padding:'9px 6px', background:active?m.bg:'rgba(255,255,255,0.04)', border:`1.5px solid ${active?m.color:'rgba(255,255,255,0.1)'}`, borderRadius:12, color:active?m.color:'rgba(255,255,255,0.5)', fontSize:11, fontWeight:700, cursor:'pointer', display:'flex', flexDirection:'column', alignItems:'center', gap:4 }}>
                  <span style={{ fontSize:18 }}>{m.icon}</span>{sub}
                </button>;
              })}
            </div>

            <div style={{ marginBottom:14 }}>
              <label style={{ color:'rgba(255,255,255,0.6)', fontSize:11, fontWeight:700, display:'block', marginBottom:5 }}>CHAPTER (OPTIONAL)</label>
            ...
            <div style={{ marginTop:'auto' }}>
              <div style={{ background:`${meta.bg}`, border:`1px solid ${meta.color}40`, borderRadius:14, padding:'14px', marginBottom:14, display:'flex', alignItems:'center', gap:12 }}>
                <span style={{ fontSize:28 }}>{meta.icon}</span>
                <div>
                  <div style={{ color:meta.color, fontWeight:800, fontSize:14 }}>{config.subject}</div>
                  <div style={{ color:'rgba(255,255,255,0.45)', fontSize:11 }}>{config.grade} • {config.syllabus} • {config.count}Q • {config.difficulty}</div>
                </div>
              </div>
              <button onClick={startQuiz} disabled={generating} style={{ width:'100%', padding:'14px', background:'linear-gradient(135deg,#ffd700,#ff9500)', border:'none', borderRadius:14, color:'#1a1a2e', fontSize:15, fontWeight:800, cursor:'pointer', opacity:generating?0.7:1, fontFamily:"'Nunito',sans-serif" }}>
      ...
      {tab==='quiz' && questions.length > 0 && (
        <div style={{ maxWidth:700, margin:'0 auto' }}>
          ...
          <div style={{ display:'flex', gap:4, marginBottom:18 }}>
            {questions.map((_,i)=>(
              <div key={i} onClick={()=>setCurrentQ(i)} style={{ flex:1, height:5, borderRadius:3, background:answers[i]!==undefined?meta.color:'rgba(255,255,255,0.12)', cursor:'pointer', transition:'background 0.2s' }}/>
            ))}
          </div>
          ...
          <div style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:20, padding:'20px 18px', marginBottom:16 }}>
            <div style={{ color:'rgba(255,255,255,0.4)', fontSize:11, fontWeight:700, marginBottom:8 }}>Q{currentQ+1}</div>
          ...
          <div style={{ display:'flex', gap:10 }}>
            {currentQ > 0 && <button onClick={()=>setCurrentQ(q=>q-1)} style={{ flex:1, padding:13, background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.12)', borderRadius:12, color:'white', fontSize:14, fontWeight:700, cursor:'pointer', fontFamily:"'Nunito',sans-serif" }}>← Prev</button>}
        ...
        <div style={{ maxWidth:700, margin:'0 auto' }}>
          <div style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:24, padding:'24px 20px', textAlign:'center', marginBottom:16 }}>
            ...
            <div style={{ color:result.score>=80?'#27ae60':result.score>=60?'#f39c12':'#e74c3c', fontSize:13, fontWeight:700, marginTop:8 }}>
              {result.score>=80?'Excellent! You\'ve mastered this!':result.score>=60?'Good job! Keep practicing.':result.score>=40?'Getting there! Review and retry.':'Study the chapter and try again.'}
            </div>
